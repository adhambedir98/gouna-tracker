...
- `sites/` is the site database: every site we run and every site we could film with, with a status pipeline (prospect, contacted, agreed, ready to film, active, paused, closed), channel (Direct Ops or Channel), hub area (the three Cairo areas, Alexandria, Mansoura, New Mansoura, Damietta), city, industry, the Portfolio Manager picked from the team (every site has one; a partner puts their own name) and, on Direct Ops sites only, the site lead, contact, phones it can take, source, last contact, notes. Opening a site shows its history: hours this month, check-ins, reports, incidents, and the people at it. Active sites are the ones on the forms.
