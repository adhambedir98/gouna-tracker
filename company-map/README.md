...
- `report/day/` is the company report, in reading order: a headline in plain words, the sites that need attention tonight with the reason, four numbers (hours today, phones active, hours per phone, opt-in rate) each with its last 30 days as a sparkline, the month as a ring with three boxes (days gone, hours a day still needed, on pace for), the site table with a week strip per site and a foot that adds up each channel and then the day, the incidents filed that day, and the incident lines from the check-outs. Three folds hold the rest for whoever wants it: the morning and the evening number by number, six charts for the last 30 days (hours per day with uploaded and still on the phones and a cell per day for sites in, phones filming against employees present, hours per phone, opt-in rate, sites in by the deadline, phones in the morning against the evening), and four charts putting site against site (hours, present against filming, hours per phone, opt-in). A fold draws its charts when it opens, and a printed copy carries them all. Every chart is SVG drawn in code (`js/charts.js`) at the width it is shown at. "Copy as text" makes a plain version for the management group; "Print" redraws the charts at paper width first. The same page manages the two deadlines, the monthly targets, the Slack webhook, the evening email, and shows the activity log. The settings panel is a founder's alone.
- `dashboard/` is the live map: Egypt drawn in code (`js/egypt.js`, with the coast and borders, the Nile and its Delta branches, the canal, the main roads, the towns, a degree grid, a scale bar, and a north arrow), every open site on it, and every phone seen in the last 14 days as a dot at the site of its latest check-in or check-out. The dot is green when the phone records 5 hours a day or more over the window (7, 14, or 30 days), yellow from 3, red under 3, grey when the window holds no evening reading for it. A phone's day is the rise in its all-time minutes since that morning's row, or since its last row when there was no morning, the same sum the evening check-out uses. A site sits on the map by its pin (latitude and longitude on the site database), else by its city, else by a place in its name, else by its hub area, and otherwise in the list only. Under the map there is one card per site: the channel, the city, the status, its green, yellow, and red counts, and its hours a phone a day. Click a card, or a site on the map, and it opens on that site's phones, one row each with its hours a day, today, the days read, when it was last seen, and its minutes, and a total row under them: that site's hours a day, its hours today, and the minutes its phones hold, each minutes total also said in hours. Six numbers sit above the map: the red, yellow, and green counts, the hours a day the whole fleet records, the hours one phone records a day, and the hours still sitting on the phones. They count every phone the database returned, including the ones at a site the map cannot place. The page redraws itself every five minutes.
