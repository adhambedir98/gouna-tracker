...
- `report/checkin/` is the morning check-in, by 9:00 AM: your name (from the team directory), the site (a person tied to one site picks it), recording started at, phones recording, employees present, phones down, any problem with one line.
- `report/` is the evening check-out, by 6:00 PM: phones deployed, employees present, phones down, a ledger with one row per phone (its tag, minutes all time, minutes still saved on it), incident yes or no with one line, what the site needs, anything else. The day's hours are worked out from the rise in each phone's all-time minutes since its last row (`dr_phone_log`). A second send for the same site and day replaces the first. Marking an incident points to the incident form.
...
- `report/day/` is the company report: a headline in plain words, then the five numbers (phones active, hours today, hours per phone, opt-in rate, present against filming) each with its last 30 days as a sparkline and a 7 day and month comparison, a ledger line for the morning and the evening, the sites that need attention tonight with the reason, the month as a ring with a projection beside thirty bars (every day a link, a cell under each showing whether every site was in), hours and present against filming by site, the site table with a week strip per site, the incidents filed that day, and the incident lines from the check-outs. Every chart is SVG drawn in code (`js/charts.js`) at the width it is shown at. "Copy as text" makes a plain version for the management group; "Print" redraws the charts at paper width first. The same page manages the codes, the two deadlines, the monthly targets, the Slack webhook, the evening email, and shows the activity log.
