...
- `dashboard/` is the live map: Egypt drawn in code (`js/egypt.js`), every open site on it, and every phone seen in the last 14 days as a dot at the site of its latest check-in or check-out. The dot is green when the phone records 5 hours a day or more over the window (7, 14, or 30 days), yellow from 3, red under 3, grey when the window holds no evening reading for it. A phone's day is the rise in its all-time minutes since that morning's row, or since its last row when there was no morning, the same sum the evening check-out uses. A site sits on the map by its pin (latitude and longitude on the site database), else by its city, else by a place in its name, else by its hub area, and otherwise in the list only. Click a site for its phones. The page redraws itself every five minutes.
