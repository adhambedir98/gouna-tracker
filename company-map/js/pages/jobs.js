...
app.content.innerHTML = index.groups.map(g => `
  <section id="${esc(g.id)}">
    ...
    <div class="cards two">${g.jobs.map(s => `<a class="card" href="${href('jobs/' + s)}"><h3>${esc(J[s] ? J[s].title : labelOf('jobs/' + s))}</h3>${J[s] ? `<p class="mute small" style="margin:6px 0 0">${esc(J[s].summary)}</p><p class="tiny accent" style="margin:8px 0 0">${L('Reports to')} ${esc(J[s].boss.replace(/^The /, 'the '))}</p>` : ''}</a>`).join('')}</div>
  </section>`).join('');
