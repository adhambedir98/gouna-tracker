...
app.content.innerHTML = `
  ...
  <p class="mute small">${esc(T('Every page you cannot open is left out of the list on the left. The three site forms need no account at all.', 'كل صفحة لا يمكنك فتحها تُحذف من القائمة على اليسار. نماذج المواقع الثلاثة لا تحتاج حسابًا.'))}</p>
  ...
  <p>${esc(T('Your name and the time sit faintly across every page you open, and they print on every copy. A picture of a page says where it came from. Printing, saving, the Print Screen key, and a copy of a whole page are logged and told to management as they happen.', 'اسمك والوقت يظهران بخفة عبر كل صفحة تفتحها، ويُطبعان مع كل نسخة. صورة الصفحة تقول من أين جاءت. الطباعة والحفظ ومفتاح طباعة الشاشة ونسخ صفحة كاملة تُسجَّل وتُبلَّغ للإدارة فور حدوثها.'))}</p>
