...
const app = await mount({
  page: 'incidents',
  title: { en: 'Incident playbooks' },
  lede: { en: 'Pick the incident. Do the steps in order. Every playbook ends with who is told and by when.' }
});
...
function playbook(p) {
  return `<section id="playbook" class="card panel" aria-live="polite">
    ...
    <h3>Who is told, by when</h3>
    <dl class="kv">${p.tell.map(x => `<dt class="k">${esc(x.by)}</dt><dd class="v"><b>${esc(x.who)}</b></dd>`).join('')}</dl>
    ${p.never ? `<p class="callout" style="margin-top:18px"><b>Never.</b> ${esc(p.never)}</p>` : ''}
...
function render(scroll) {
  ...
  app.content.innerHTML = `
    ...
    ${p ? playbook(p) : `<p class="mute" style="margin-top:20px">Nine playbooks. Each one: the first thing to do, the steps with an owner and a clock, and who is told.</p>`}
