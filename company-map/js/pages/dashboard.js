...
function phonesHTML(id) {
  ...
  return `<div class="t-wrap"><table class="t dash phones"><thead><tr><th>${esc(L('Phone'))}</th><th>${esc(L('Hours a day'))}</th><th class="num">${esc(L('Today'))}</th><th class="num">${esc(L('Days read'))}</th><th>${esc(L('Last seen'))}</th><th class="num">${esc(L('Minutes all time'))}</th><th class="num">${esc(L('Minutes saved locally'))}</th></tr></thead>
    ...
    <tfoot><tr><th>${esc(L('All {n} phones', { n: n(ph.length) }))}</th><td>${any(ph, 'hours_day') ? esc(hrs(sum(ph, 'hours_day'))) : ''}</td><td class="num">${any(ph, 'today') ? esc(hrs(sum(ph, 'today'))) : ''}</td><td></td><td></td>
...
function render() {
  ...
  // a day is what the month is built from, the hours a phone is whether that is healthy, and the hours still on the phones
  // are the ones nobody can use yet. A count of nothing is grey, so the eye lands on the ones that need a call.
  app.content.innerHTML = `
    ...
    <p class="tiny dim">${esc(L('{p} phones at {s} sites.', { p: n(all.length), s: n(sites.filter(x => Number(x.phones)).length) }))}${quiet ? ' ' + esc(L('{n} have sent no reading in this window.', { n: n(quiet) })) : ''}${late ? ' ' + esc(L('{n} sites have phones but no check-in today.', { n: n(late) })) : ''}${localHours ? ' ' + esc(L('Hours still on the phones is what each one was holding when it was last read.')) : ''}${wrong ? ' ' + esc(wrong === 1 ? L('One reading says more is saved on the phone than it has ever recorded, so it was typed wrong.') : L('{n} readings say more is saved on the phone than it has ever recorded, so they were typed wrong.', { n: n(wrong) })) : ''}</p>
