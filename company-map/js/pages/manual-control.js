...
const app = await mount({
  ...
  title: { en: m.title },
  lede: { en: m.purpose },
  toc: [{ id: 'dashboard', label: { en: 'The dashboard' } }, { id: 'alerts', label: { en: 'Alerts' } }, { id: 'calculator', label: { en: 'Capacity calculator' } }, ...MANUAL_TOC]
});
const R = m.ratios;
...
function dashboard() {
  ...
  s += text(14, 21, 'Control', { cls: 'tx tx-b' });
  s += text(346, 21, '8 PM number: on the plan', { cls: 'tx tx-s tx-m', anchor: 'end' });
  // MDM map panel
  s += rect(12, 38, 166, 150, 'bx');
  s += text(20, 54, 'MDM map', { cls: 'tx tx-b tx-s' });
  ...
  s += text(20, 180, 'filled: uploading. hollow: dark', { cls: 'tx tx-d tx-s' });
  // QC verdicts panel
  s += rect(186, 38, 162, 70, 'bx');
  s += text(194, 54, 'QC verdicts', { cls: 'tx tx-b tx-s' });
  [['keep', 118], ['feedback', 28], ['fraud', 10]].forEach(([k, w], i) => {
    const y = 62 + i * 13;
    s += text(194, y + 9, k, { cls: 'tx tx-s tx-m' });
    s += rect(246, y + 1, w * 0.8, 9, i === 0 ? 'bx-acc' : i === 1 ? 'bx-acc-line' : 'bx-panel');
  });
  // hours panel
  s += rect(186, 118, 162, 70, 'bx');
  s += text(194, 134, 'Hours against quota', { cls: 'tx tx-b tx-s' });
  ...
  s += text(344, 141, 'quota', { cls: 'tx tx-s tx-a', anchor: 'end' });
  // the join
  s += rect(12, 196, 336, 20, 'bx-soft');
  s += text(180, 210, 'joined by the device number and the wearer log', { cls: 'tx tx-s tx-a', anchor: 'middle' });
  // alerts strip
  s += rect(12, 224, 336, 82, 'bx-panel');
  s += text(20, 240, 'Alerts', { cls: 'tx tx-b tx-s' });
  [['12:04', 'Phone 214 dark since clock-in', 'anchor'], ['16:40', 'Site C, 1.3 device-days of backlog', 'Moharam'], ['17:12', 'Flag on session 8812, torso angle', 'Moharam, QC']].forEach(([tm, msg, to], i) => {
    ...
    s += text(56, y, msg, { cls: 'tx tx-s' });
    s += text(340, y, to, { cls: 'tx tx-s tx-a', anchor: 'end' });
  });
  return figure(svg({ w: W, h: H, label: 'A sketch of the control dashboard: MDM map, QC verdicts, hours count, alerts', inner: s }), { caption: 'A sketch. The numbers are examples.', cls: 'narrow' });
}
...
function readout(hours) {
  ...
  return cells.map(([n, l]) => `<div><div class="big">${fmt(n)}</div><div class="lbl">${l}</div></div>`).join('');
}

function render() {
  app.content.innerHTML = `
    <section id="dashboard">
      <h2>One screen, three feeds</h2>
      ${dashboard()}
    ...
    <section id="calculator">
      ...
      <div class="field" style="max-width:260px"><label for="hours">Hours a day</label><input type="number" id="hours" min="0" step="50" value="${R.per}" inputmode="numeric"></div>
      <div class="readout" id="readout">${readout(R.per)}</div>
      <p class="mute small">Per ${fmt(R.per)} hours a day: about ${R.phones} phones, ${R.anchors} anchors, ${R.supervisors} supervisors, ${R.reviewers} reviewers, ${R.mbps} Mbps. A hub passes at ${R.hubMbps} Mbps, always at least two. Hats are one per phone plus one spare per ten. Spares are ${Math.round(R.spareRate * 100)}% of deployed phones.</p>
    </section>
    ${blocks(m)}`;
}
render();
