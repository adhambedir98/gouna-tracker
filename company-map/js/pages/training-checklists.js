...
const L = { pick: { en: 'Pick the role', ar: 'اختر الدور' }, passes: { en: 'Passes when', ar: 'ينجح عندما' }, print: { en: 'Print this checklist', ar: 'اطبع هذه القائمة' }, guide: { en: 'The guide', ar: 'الدليل' }, quiz: { en: 'The quiz', ar: 'الاختبار' } };
...
function render() {
  ...
  app.content.innerHTML = `<section id="roles"><h2>${esc(t(L.pick))}</h2><div class="choices roles">${data.roles.map(x => `<button type="button" data-role="${x.id}" class="${x.id === role ? 'on' : ''}">${esc(t(x.role))}</button>`).join('')}</div></section>${list(r)}`;
}
render();
onLang(render);
