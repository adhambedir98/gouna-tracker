...
app.content.innerHTML = `
  <div class="cards two">${(data.tools || []).map(t => `<section class="card" id="${esc(t.id)}">
    <h3>${esc(t.name)} <span class="pill ${cls[t.status] || ''}">${esc(STATUS[t.status] || t.status)}</span></h3>
    <p>${esc(t.what)}</p>
    <ul>${(t.lines || []).map(l => `<li>${esc(l)}</li>`).join('')}</ul>
    <p class="tiny mute">${esc(L('Runs it'))}: ${esc(t.owner)}</p>
  </section>`).join('')}</div>
  ${(data.rules || []).length ? `<section><h2>${esc(L('The rules'))}</h2><ul class="rows">${data.rules.map(r => `<li>${esc(r)}</li>`).join('')}</ul></section>` : ''}
  ${(data.faq || []).length ? `<section><h2>${esc(L('Questions'))}</h2>${data.faq.map((q, i) => detailsHTML({ id: 'q' + i, title: q.q, body: `<p>${esc(q.a)}</p>` })).join('')}</section>` : ''}`;
