import { mount, loadJSON, t, esc, store, onLang, initialHash, setHash, href, site } from '../app.js';
...
function quiz(s) {
  ...
  return `<section id="quiz" class="card panel qz">
    ...
    <div class="signoff"><div>${esc(ui('trainee'))}</div><div>${esc(ui('trainer'))}</div><div>${esc(ui('date'))}</div><div>${esc(t({ en: 'Score', ar: 'النتيجة' }))}</div></div>
...
function render() {
  ...
  app.content.innerHTML = `<section id="pick"><h2>${esc(t(L.pick))}</h2><div class="choices roles">${sets.map(x => `<button type="button" data-set="${x.id}" class="${x.id === cur ? 'on' : ''}">${esc(t(x.title))}</button>`).join('')}</div></section>${quiz(s)}`;
}
render();
onLang(render);
