...
function render() {
  ...
  app.content.innerHTML = `
    <div class="stat dash-stat">
      ...
      <div><div class="big num${loud.length ? ' bad' : ''}">${fmt(loud.length)}</div><div class="lbl">${esc(L('copy and print signals in the last 120 events'))}</div></div>
    </div>
...
const watch = () => `<div class="t-wrap"><table class="t reg"><thead><tr><th>${esc(L('When'))}</th><th>${esc(L('Person'))}</th><th>${esc(L('What'))}</th><th>${esc(L('Page'))}</th></tr></thead>
  ...
  <p class="tiny dim">${esc(L('A browser is never told that a screenshot was taken, on any browser, and a photograph of the screen leaves no trace at all. These are the signals a browser does give. The ones in red are sent to Slack and to the report email the moment they happen; the rest are only written down here. The name across every page is what makes a leaked picture traceable.'))}</p>`;
